...
WebInspector.FlameChart.prototype = {
    ...
    setWindowTimes: function(startTime, endTime)
    {
        ...
        this._cancelWindowTimesAnimation = WebInspector.animateFunction(this.element.window(), this._animateWindowTimes.bind(this), [{
            from: this._timeWindowLeft,
            to: startTime
        }, {
            from: this._timeWindowRight,
            to: endTime
        }], 5, this._animationCompleted.bind(this));
    ...
    _draw: function(width, height)
    {
        ...
        for (var i = 0; i < nextTitleIndex; ++i) {
            ...
            if (this._dataProvider.decorateEntry(entryIndex, context, text, barX, barY, barWidth, barHeight))
                ...
                continue;context.fillStyle = this._dataProvider.textColor(entryIndex);
    ...
    _drawFlowEvents: function(context, width, height)
    {
        ...
        for (var i = 0; i < endIndex; ++i) {
            if (flowEndTimes[i] < timeWindowLeft)
                continue;var startX = this._timeToPosition(flowStartTimes[i]);
            ...
            if (endX - startX < minimumFlowDistancePx)
                ...
                continue;if (endX - startX < minimumFlowDistancePx + fadeColorsRange || colorIndex !== color.length - 1) {
...
WebInspector.OverviewGrid.Window = function(parentElement, dividersLabelBarElement)
{
    ...
    this._parentElement.appendChild(WebInspector.Widget.createStyleElement("ui_lazy/overviewGrid.css"));
...
WebInspector.TimelineGrid.calculateDividerOffsets = function(calculator, freeZoneAtLeft)
{
    ...
    return {
        offsets: offsets,
        precision: Math.max(0, -Math.floor(Math.log(gridSliceTime * 1.01) / Math.LN10))
    };
}
    ...
    _updateMarkers: function()
    {
        ...
        for (var time of this._markers.keys()) {
            ...
            if (filteredMarkers.has(position))
                continue;filteredMarkers.set(position, marker);
