...
WebInspector.ProfileDataGridNode.prototype = {
    createCell: function(columnIdentifier)
    {
        ...
        if (this.profileNode.scriptId !== "0") {
            ...
            var callFrame = (this.profileNode);
...
WebInspector.TopDownProfileDataGridNode._sharedPopulate = function(container)
{
    ...
    for (var i = 0; i < childrenLength; ++i)
        container.appendChild(new WebInspector.TopDownProfileDataGridNode(children[i],(container.tree)));
...
WebInspector.CPUProfileView.prototype = {
    ...
    _getBottomUpProfileDataGridTree: function()
    {
        if (!this._bottomUpProfileDataGridTree)
            this._bottomUpProfileDataGridTree = new WebInspector.BottomUpProfileDataGridTree(this,(this.profile.profileHead));
        return this._bottomUpProfileDataGridTree;
    },
    _getTopDownProfileDataGridTree: function()
    {
        if (!this._topDownProfileDataGridTree)
            this._topDownProfileDataGridTree = new WebInspector.TopDownProfileDataGridTree(this,(this.profile.profileHead));
    ...
    _onEntrySelected: function(event)
    {
        ...
        if (!script)
            return;
        var location = (debuggerModel.createRawLocation(script, node.lineNumber - 1, node.columnNumber ? node.columnNumber - 1 : node.columnNumber));
...
WebInspector.CPUProfileType.prototype = {
    ...
    {
        var protocolId = (event.data.protocolId);
        var scriptLocation = (event.data.scriptLocation);
        var resolvedTitle = (event.data.title);
        if (!resolvedTitle) {
    ...
    _consoleProfileFinished: function(event)
    {
        var protocolId = (event.data.protocolId);
        var scriptLocation = (event.data.scriptLocation);
        var cpuProfile = (event.data.cpuProfile);
        var resolvedTitle = (event.data.title);
    ...
    _addMessageToConsole: function(type, scriptLocation, messageText)
    {
        ...
        var message = new WebInspector.ConsoleMessage(target,WebInspector.ConsoleMessage.MessageSource.ConsoleAPI,WebInspector.ConsoleMessage.MessageLevel.Debug,messageText,type,undefined,undefined,undefined,undefined,undefined,[{
            functionName: "",
    ...
    stopRecordingProfile: function()
    {
        ...
        this._profileBeingRecorded.target().cpuProfilerModel.stopRecording().then(didStopProfiling.bind(this)).then(WebInspector.targetManager.resumeAllTargets.bind(WebInspector.targetManager)).then(fireEvent.bind(this));
    },
    createProfileLoadedFromFile: function(title)
    {
...
WebInspector.HeapSnapshotViewportDataGrid.prototype = {
    ...
    _addVisibleNodes: function(parentNode, topBound, bottomBound)
    {
        ...
        for (var i = 0; i < children.length; ++i) {
            ...
            if (nameFilterValue && child.filteredOut && child.filteredOut(nameFilterValue))
                continue;var newTop = topPadding + this._nodeHeight(child);
            if (newTop > topBound)
                break;
            topPadding = newTop;
        }
        ...
        for (; i < children.length && position < bottomBound; ++i) {
            var child = children[i];
            if (nameFilterValue && child.filteredOut && child.filteredOut(nameFilterValue))
                continue;var hasChildren = child.hasChildren;
    ...
    revealTreeNode: function(pathToReveal)
    {
        ...
        var node = (pathToReveal.peekLast());
        var scrollTop = this.scrollContainer.scrollTop;
...
WebInspector.HeapSnapshotConstructorNode.prototype = {
    ...
    populateNodeBySnapshotObjectId: function(snapshotObjectId)
    {
        ...
        {
            var node = (this.childForPosition(nodePosition));
...
WebInspector.HeapSnapshotView.prototype = {
    ...
    performSearch: function(searchConfig, shouldJump, jumpBackwards)
    {
        var nextQuery = new WebInspector.HeapSnapshotCommon.SearchConfig(searchConfig.query.trim(),searchConfig.caseSensitive,searchConfig.isRegex,shouldJump,jumpBackwards || false);
...
WebInspector.SaveSnapshotOutputStreamDelegate.prototype = {
    ...
    {
        WebInspector.console.error("Failed to read heap snapshot from temp file: " + (event).message);
...
WebInspector.HeapTrackingOverviewGrid.prototype = {
    ...
    _setSamples: function(samples)
    {
        ...
        this._profileSamples.totalTime = (samples.timestamps.peekLast());
        this.update();
    },
