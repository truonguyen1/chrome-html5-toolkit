...
WebInspector.AllocationProfile.prototype = {
    _buildFunctionAllocationInfos: function(profile)
    {
        ...
        for (var i = 0; i < infoLength; i += functionInfoFieldCount) {
            functionInfos[index++] = new WebInspector.FunctionAllocationInfo(strings[rawInfos[i + functionNameOffset]],strings[rawInfos[i + scriptNameOffset]],rawInfos[i + scriptIdOffset],rawInfos[i + lineOffset],rawInfos[i + columnOffset]);
        }
    },
    _buildAllocationTree: function(profile, liveObjectStats)
    {
        ...
        function traverseNode(rawNodeArray, nodeOffset, parent)
        {
            ...
            var result = new WebInspector.TopDownAllocationNode(id,functionInfo,rawNodeArray[nodeOffset + allocationCountOffset],rawNodeArray[nodeOffset + allocationSizeOffset],liveCount,liveSize,parent);
    ...
    serializeTraceTops: function()
    {
        ...
        for (var i = 0; i < functionInfos.length; i++) {
            ...
            if (info.totalCount === 0)
                continue;var nodeId = this._nextNodeId++;
            var isRoot = i == 0;
    ...
    serializeAllocationStack: function(traceNodeId)
    {
        ...
        while (node) {
            ...
            result.push(new WebInspector.HeapSnapshotCommon.AllocationStackFrame(functionInfo.functionName,functionInfo.scriptName,functionInfo.scriptId,functionInfo.line,functionInfo.column));
            node = node.parent;
        }
        return result;
    },
...
WebInspector.HeapSnapshot.prototype = {
        ...
        while (index < nodesToVisitLength) {
            ...
            for (var edgeIndex = firstEdgeIndex; edgeIndex < edgesEnd; edgeIndex += edgeFieldsCount) {
                ...
                if (edgeType === edgeWeakType)
                    continue;var childNodeIndex = containmentEdges[edgeIndex + edgeToNodeOffset];
                var childNodeOrdinal = childNodeIndex / nodeFieldCount;
                if (distances[childNodeOrdinal] !== noDistance)
                    continue;edge.edgeIndex = edgeIndex;
                if (filter && !filter(node, edge))
                    continue;distances[childNodeOrdinal] = distance;
    ...
    _buildAggregates: function(filter)
    {
        ...
        for (var nodeIndex = 0; nodeIndex < nodesLength; nodeIndex += nodeFieldCount) {
            ...
            if (flags && !(flags[nodeOrdinal] & flag))
                continue;node.nodeIndex = nodeIndex;
            if (filter && !filter(node))
                continue;var selfSize = nodes[nodeIndex + selfSizeOffset];
            if (!selfSize && nodes[nodeIndex + nodeTypeOffset] !== nodeNativeType)
                continue;var classIndex = node.classIndex();
    ...
    _calculateClassesRetainedSize: function(aggregates, filter)
    {
        ...
        while (list.length) {
            ...
            if (!seen && (!flags || (flags[nodeOrdinal] & flag)) && (!filter || filter(node)) && (node.selfSize() || nodes[nodeIndex + nodeTypeOffset] === nodeNativeType)) {
    ...
    _buildPostOrderIndex: function()
    {
        ...
        while (true) {
            ...
            while (stackTop >= 0) {
                ...
                if (edgeIndex < edgesEnd) {
                    ...
                    if (visited[childNodeOrdinal])
                        continue;var nodeFlag = !flags || (flags[nodeOrdinal] & flag);
            ...
            stackTop = 0;
            stackNodes[0] = rootNodeOrdinal;
            stackCurrentEdge[0] = firstEdgeIndexes[rootNodeOrdinal + 1];
            for (var i = 0; i < nodeCount; ++i) {
                if (!visited[i]) {
        ...
        if (postOrderIndex !== nodeCount) {
            ...
            for (var i = 0; i < nodeCount; ++i) {
                if (visited[i])
                    continue;dumpNode.nodeIndex = i * nodeFieldCount;
                errors.addError(dumpNode.name() + " @" + dumpNode.id());
    ...
    _buildDominatorTree: function(postOrderIndex2NodeOrdinal, nodeOrdinal2PostOrderIndex)
    {
        ...
        var nodeOrdinal;
        {
            ...
            for (var edgeIndex = firstEdgeIndexes[nodeOrdinal]; edgeIndex < endEdgeIndex; edgeIndex += edgeFieldsCount) {
                var edgeType = containmentEdges[edgeIndex + edgeTypeOffset];
                if (edgeType === edgeWeakType || edgeType === edgeShortcutType)
                    continue;var childNodeOrdinal = containmentEdges[edgeIndex + edgeToNodeOffset] / nodeFieldCount;
        ...
        while (changed) {
            ...
            for (var postOrderIndex = rootPostOrderedIndex - 1; postOrderIndex >= 0; --postOrderIndex) {
                if (affected[postOrderIndex] === 0)
                    continue;affected[postOrderIndex] = 0;
                if (dominators[postOrderIndex] === rootPostOrderedIndex)
                    continue;nodeOrdinal = postOrderIndex2NodeOrdinal[postOrderIndex];
                ...
                for (var retainerIndex = beginRetainerIndex; retainerIndex < endRetainerIndex; ++retainerIndex) {
                    ...
                    if (retainerNodeIndex !== rootNodeIndex && nodeFlag && !retainerNodeFlag)
                        continue;var retanerPostOrderIndex = nodeOrdinal2PostOrderIndex[retainerNodeOrdinal];
    ...
    _buildSamples: function()
    {
        ...
        for (var nodeIndex = 0; nodeIndex < nodesLength; nodeIndex += nodeFieldCount) {
            ...
            var nodeId = node.id();
            if (nodeId % 2 === 0)
                continue;var rangeIndex = lastAssignedIds.lowerBound(nodeId);
...
WebInspector.HeapSnapshotEdgesProvider = function(snapshot, filter, edgesIter, indexProvider)
{
    ...
    var iter = filter ? new WebInspector.HeapSnapshotFilteredIterator(edgesIter,(filter)) : edgesIter;
...
WebInspector.HeapSnapshotLoader.prototype = {
    ...
    _writeBalancedJSON: function(data)
    {
        ...
        this._snapshot.snapshot = (JSON.parse(data));
    }
};
    ...
    _markQueriableHeapObjects: function()
    {
        ...
        while (list.length) {
            ...
            if (flags[nodeOrdinal] & flag)
                continue;flags[nodeOrdinal] |= flag;
            ...
            for (var edgeIndex = beginEdgeIndex; edgeIndex < endEdgeIndex; edgeIndex += edgeFieldsCount) {
                ...
                if (flags[childNodeOrdinal] & flag)
                    continue;var type = containmentEdges[edgeIndex + edgeTypeOffset];
    ...
    _markPageOwnedNodes: function()
    {
        ...
        for (var edgeIndex = firstEdgeIndexes[rootNodeOrdinal], endEdgeIndex = firstEdgeIndexes[rootNodeOrdinal + 1]; edgeIndex < endEdgeIndex; edgeIndex += edgeFieldsCount) {
            ...
            if (edgeType === edgeElementType) {
                ...
                continue;var nodeOrdinal = nodeIndex / nodeFieldCount;
        ...
        while (nodesToVisitLength) {
            ...
            for (var edgeIndex = beginEdgeIndex; edgeIndex < endEdgeIndex; edgeIndex += edgeFieldsCount) {
                ...
                if (type === edgeWeakType)
                    continue;nodesToVisit[nodesToVisitLength++] = childNodeOrdinal;
    ...
    _calculateArraySize: function(node)
    {
        ...
        for (var edgeIndex = beginEdgeIndex; edgeIndex < endEdgeIndex; edgeIndex += edgeFieldsCount) {
            ...
            if (edgeType !== edgeInternalType)
                continue;var edgeName = strings[containmentEdges[edgeIndex + edgeNameOffset]];
            if (edgeName !== "elements")
                continue;var elementsNodeIndex = containmentEdges[edgeIndex + edgeToNodeOffset];
